...
// New quantities for products
foreach ($newQuantities as $product => $quantity) {
$sql = <<<SQL UPDATE inventory SET quantity=CASE WHEN product_name='$product' THEN $quantity ELSE quantity END WHERE
    product_name='$product' ; SQL; if ($conn->query($sql) === TRUE) {
